...
from config.double_dqn_config import *
...
def train_double_dqn():
    ...
    policy_net = DoubleDQN(input_channels=FRAME_STACK, num_actions=NUM_ACTIONS)
    target_net = DoubleDQN(input_channels=FRAME_STACK, num_actions=NUM_ACTIONS)
    
    import config.double_dqn_config as config
    agent = DoubleDQNAgent(policy_net, target_net, config)
    
    replay_buffer = ReplayBuffer(capacity=REPLAY_BUFFER_SIZE)
    
    logger = DQNLogger(LOG_DIR, "double_dqn")
    logger.save_config({
    ...
    for episode in tqdm(range(TOTAL_EPISODES), desc="Training Double DQN"):
