...
from config.prioritized_dqn_config import *
from models.prioritized_dqn import PrioritizedDQN
...
# Directories
LOG_DIR = "logs/prioritized_dqn"
CHECKPOINT_DIR = "checkpoints/prioritized_dqn"
...
def train_prioritized_dqn():
    ...
    policy_net = PrioritizedDQN(input_channels=FRAME_STACK, num_actions=NUM_ACTIONS)
    target_net = PrioritizedDQN(input_channels=FRAME_STACK, num_actions=NUM_ACTIONS)
    
    import config.prioritized_dqn_config as config
    agent = PrioritizedDQNAgent(policy_net, target_net, config)
    
    # Use Prioritized Replay Buffer instead of standard ReplayBuffer
    ...
    logger = DQNLogger(LOG_DIR, "prioritized_dqn", enabled=False)
    logger.save_config({
        'algorithm': 'Prioritized DQN',
    ...
    for episode in tqdm(range(TOTAL_EPISODES), desc="Training Prioritized DQN"):
